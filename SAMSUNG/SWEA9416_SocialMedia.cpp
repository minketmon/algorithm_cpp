...
int following[1005][1005], post_cnt;
int post[100000], uploader[100000], post_like[100000];

void init(int N) {
    n = N, post_cnt = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            if (i == j) following[i][j] = 1;
            else following[i][j] = 0;
        }
    }
    memset(post_like, 0, sizeof(post_like));
}

void follow(int uID1, int uID2, int timestamp) {
    following[uID1][uID2] = timestamp;
}
...
void getFeed(int uID, int timestamp, int pIDList[]) {
    priority_queue<pair<int, int>> pq1; // 1000초 이내인 게시글
    int lower = lower_bound(post, post + post_cnt + 1, timestamp - 1000) - post;
    for (int i = lower; i <= post_cnt; i++) {
        if (following[uID][uploader[i]] > 0) pq1.push({post_like[i], i});
    }
    int idx = 0;
    while (!pq1.empty()) {
        if (idx == 10) return;
        pIDList[idx++] = pq1.top().second;
        pq1.pop();
    }
    for(int i = lower-1; i>=0; i--){
        if(idx == 10) return;
        if(following[uID][uploader[i]] > 0) pIDList[idx++] = i;
    }
}
...
//            for (int i = 0; i < 10; i++) cout << pIdList[i] << ' ';
//            cout << '\n';
            bool print = true;
            for (int i = 0; i < 10; i++) {
                if (ans_pIdList[i] == 0)
            ...
            if (!print) {
                ...
                for (int i = 0; i < 10; i++) cout << pIdList[i] << ' ';
                cout << '\n';
                for (int i = 0; i < 10; i++) cout << ans_pIdList[i] << ' ';
                cout << '\n';
                cout << "--------------------\n";
            }
        }
